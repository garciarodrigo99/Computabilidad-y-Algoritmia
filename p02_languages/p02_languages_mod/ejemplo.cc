...
Language ReadLanguage(std::string string, int start_position, Language &lang) {
  while (SplitChain(string, kDelimeter).at(start_position).at(0) !=
         SET_CLOSER) {
    Chain chain;
    ...
    if (!((SplitChain(string, kDelimeter).at(start_position).size() == 1) &&
          (SplitChain(string, kDelimeter).at(start_position).at(0) ==
           kEmptyChain))) {
      for (size_t i = 0;
           i < SplitChain(string, kDelimeter).at(start_position).size(); i++) {
        std::string string_aux;
        string_aux.push_back(
            SplitChain(string, kDelimeter).at(start_position).at(i));
...
void information(char *p_name) {
  std::cout
      ...
      << p_name
      << " ficheroentrada.txt ficheroentrada2.txt ficherosalida.txt"
         " opcode\n\n"
         "ficheroentrada.txt\tFichero de texto que contiene los simbolos de un "
         "alfabeto y un lenguaje\n"
         "ficheroentrada2.txt\tFichero de texto que contiene los simbolos de "
         "un "
         "alfabeto y un lenguaje\n"
...
int main(int argc, char *argv[]) {
  ...
  if ((atoi(argv[argc - 1]) < 1) || (atoi(argv[argc - 1]) > kOpcode)) {
    std::cout << "¡Opcode no válido!\n\n"
                 "Introducir valores entre "
              << 1 << " y " << kOpcode << std::endl;
    return 2;
  }
  ...
  bool operacion_unaria =
      ((atoi(argv[argc - 1]) == 5) || (atoi(argv[argc - 1]) == 6));
